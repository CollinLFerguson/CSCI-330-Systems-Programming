...
NODE *CREATE_NODE(NODE *root, int value); //returns the created node so that it can be more easily linked.
void LOAD(LINK *(*head),NODE *(*root), FILE *stream, int arrayLen);
void PRINT_INORDER(NODE *root);
void PRINT_PREORDER(NODE *root);
void PRINT_POSTORDER(NODE *root);
void PRINT_LIST(LINK *head);
void GET_MIDDLE(int array[], int start, int end, int reordered[]);
